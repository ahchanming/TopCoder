...
class TestTaking {
    ...
    int findMax(int questions, int guessed, int actual) {
        int result = 0;
        int realTrue = min(guessed, actual);
        result += realTrue;
        int answerWrong = questions - guessed;
        int realWrong = questions - actual;
        result += min(answerWrong, realWrong);
        return result;
    }
};
